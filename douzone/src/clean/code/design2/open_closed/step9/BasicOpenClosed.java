package clean.code.design2.open_closed.step9;
...
class AndComparable implements Comparable<Product> {
	Comparable<Product> first;
	Comparable<Product> second;

    AndComparable(Comparable<Product> first, Comparable<Product> second) {
    	this.first = first;
    	this.second = second;
    }
...
public class BasicOpenClosed {
	...
	public static void main(String[] args) {
	    ...
	    AndComparable green_large_things = new AndComparable(green, size);
	    List<Product> green_largeItems = betterFilter.filter(all, green_large_things);
	    for (Product item : green_largeItems) {
			System.out.println(item.getName() + " is green Color and large size");
	    }
